...
extract_tool_name() {
    ...
    tool_name=${tool_name:0:${#tool_name}-17}
    echo "$tool_name"
}
...
process_tasks() {
	...
	local line_with_version=$(grep -m 1 -- "-states" "$path_to_file")
	if [ -z "$line_with_version" ]; then
		...
		if [[ "$(uname)" == "Darwin" ]]; then
			sed -i '' "s/$version-states/states/g" $path_to_file
