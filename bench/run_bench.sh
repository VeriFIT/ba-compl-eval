...
if [ -z "$1" ]; then
  benchmarks=("advanced-automata" "s1s-direct-red" "from_ltl_red")
else
...
for benchmark in "${benchmarks[@]}"; do
	...
	cat "$benchmark.input" | ./pycobench -c omega-compl.yaml -j $j_value -t $s_value --memout $m_value -m "$tool" -o "$TASKS_FILE"
